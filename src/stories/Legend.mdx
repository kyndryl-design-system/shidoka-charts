...
You can capture legend item click events using the `onItemClick` callback:

```javascript
const pluginOpts = {
  containerId: 'my-html-legend-container',
  onItemClick: (info) => {
    console.log('Legend item clicked:', info.label);
    console.log('Current visibility:', info.isHidden ? 'Hidden' : 'Visible');
    console.log('Dataset index:', info.datasetIndex);
  },
};
